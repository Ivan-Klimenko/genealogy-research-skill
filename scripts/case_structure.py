...
def analyse(code, meta):
    ...
    # 🔴 КНИГА ВНЕ СРОКА ОПИСИ — ЛИБО НАСТОЯЩАЯ НАХОДКА, ЛИБО МУСОР, И РАЗЛИЧАЕТ
    # ИХ СТИЛЬ ШАПКИ. Там, где шапка стоит раз на книгу (д.493), год вне описи
    # почти всегда настоящий: у книги на 1838 г. он вписан в печатный бланк,
    # и именно в ней ищется венчание. Там, где шапка бежит по каждому развороту,
    # одиночный выброс — это распознавание, пережившее сглаживание.
    # Проверено прочёсом 2026-08-08: девять расхождений «заявленный год против
    # вычисленной книги» дали НОЛЬ ошибок источника и девять ошибок структуры,
    # и все девять были либо вне описи, либо на границе книг.
    if dense and lo is not None:
        runs = [r for r in runs
                if lo <= r[1] <= hi or (r[2] - r[0]) >= 4]
    ...
    for _, y, _, p in runs:
        if p:
            parts[y].add(p)
    # 🔴 «ЧАСТИ НЕТ» ОСМЫСЛЕННО ТОЛЬКО ДЛЯ ГОДОВ, КОТОРЫЕ ДЕЛО ОБЯЗАНО СОДЕРЖАТЬ.
    # Год-призрак, переживший сглаживание (в деле 1859—1861 гг. «книга на 1850»),
    # приносил с собой «нет ни одной части» и через признак 17 обвинял чистые
    # отрицания. Проверено прочёсом 2026-08-08: из десяти срабатываний признака
    # шесть были такими призраками.
    missing = {}

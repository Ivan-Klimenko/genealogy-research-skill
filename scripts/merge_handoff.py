...
hypotheses:     # поля: {', '.join(HYP_FIELDS)}
                ...
                # 🔴 evidence_for и evidence_against — СПИСКИ, по доводу на пункт,
                #    а НЕ один абзац строкой. Проверено дважды за один наряд: две ветки
                #    из шести написали абзацем, восемь гипотез отклонились и вносились
                #    руками. Абзац из «① … ② … ③ …» — это тот же список, только машине
                #    невидимый: его нельзя ни пересчитать, ни снять по одному.

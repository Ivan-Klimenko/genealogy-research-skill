...
# --- stub-узлы -------------------------------------------------------------
# 🔴 ФЛАГ `stub` ОСТАЁТСЯ АВТОРСКИМ, А НЕ ВЫЧИСЛЯЕМЫМ, И ЭТО РЕШЕНИЕ ВЛАДЕЛЬЦА
# (2026-08-14). Он говорит не о человеке, а о НАШЕЙ работе — «собственного
# исследования по нему не велось», — и бывает верен при богатых данных: узел
# может обрасти документами попутно, ради чужой связи, так и не став предметом
# исследования. Вычислять такое значит подменять утверждение измерением.
# ⚠️ НО РУЧНОЕ УТВЕРЖДЕНИЕ О СОБСТВЕННЫХ ДАННЫХ РЖАВЕЕТ МОЛЧА — это в проекте,
# из которого вырос навык, проверено трижды (счётчики, `visible`, и сам `stub`:
# на 2026-08-14 из 36 помеченных узлов 26 давно перестали быть заготовками,
# причём худший случай — женщина с 15 источниками, своими датами и тремя живыми
# задачами, чья карточка печатала «известна только по имени в чужой записи»).
# ⇒ Флаг пишется рукой, а СОГЛАСОВАННОСТЬ ЕГО С ДАННЫМИ ПЕРЕЗАДАЁТСЯ КАЖДЫЙ ПРОГОН.
# Признак заготовки: НЕ БОЛЬШЕ ОДНОГО ИСТОЧНИКА И НЕТ СОБСТВЕННЫХ ДАТ. Оба — следы
# того, что работа велась: собственная дата берётся только из записи о самом человеке,
# а второй источник означает, что его искали, а не встретили попутно.
# 🔴 НАЛИЧИЕ ЗАДАЧИ В ПРИЗНАК НЕ ВХОДИТ, И ЭТО ИСПРАВЛЕНИЕ ПЕРВОЙ РЕДАКЦИИ, СДЕЛАННОЕ
# В ТОТ ЖЕ ЧАС. Первая редакция считала живую задачу следом работы — и признак тут же
# сам себе противоречил: задача, заведённая ЧТОБЫ РАЗОБРАТЬ заготовки, мгновенно снимала
# флаг с каждой из них, и десять узлов разом объявились «протухшими». Задача — это план
# работы, а не её след; заготовка с ходом остаётся заготовкой, пока ход не отработан.
# ⚠️ Закрытая задача не входит тем более: безрезультатный ход не превращает
# неисследованного человека в исследованного.
def _looks_like_stub(p):
    return (len(p.get("evidence") or []) <= 1
            and not p.get("birth_date") and not p.get("death_date"))
...
for p in people:
    ...
    if p.get("stub"):
        ...
        if deg == 0 and p["id"] not in (graph["meta"].get("detached_branches") or {}):
            errors.append(f"person {p['id']}: stub без единой связи — он ни к чему не привязан")
        if not _looks_like_stub(p):
            why = []
            if len(p.get("evidence") or []) > 1:
                why.append(f"{len(p['evidence'])} источников")
            if p.get("birth_date") or p.get("death_date"):
                why.append("свои даты")
            errors.append(
                f"person {p['id']}: stub=true, но {', '.join(why)} — флаг протух. "
                f"Карточка печатает «известен только по имени в чужой записи», "
                f"а это уже неправда")
    elif _looks_like_stub(p):
        errors.append(
            f"person {p['id']}: не помечен stub, а признаки заготовки налицо "
            f"(источников {len(p.get('evidence') or [])}, своих дат нет). "
            f"Либо ставь stub, либо ищи по нему — иначе человек молча выпадет из работы")
...
for og in _orgs:
    ...
    # 🔴 КТО ВПРАВЕ СПРАШИВАТЬ — вопрос отдельный от «что учреждение умеет»,
    # и у каждого вида учреждения на него свой закон. Архивы живут по ч.3 ст.25
    # ФЗ-125 (75 лет со дня создания документа, после — родство не требуется),
    # ЗАГС по ст.9 ФЗ-143 (свой круг лиц), дела репрессированных по закону
    # о реабилитации, ведомственные хранилища — вне правила 75 лет вовсе.
    # Не записав этого, заявитель принимает названную учреждением норму на веру,
    # хотя к его документу она может не относиться.
    if og.get("kind") in ("архив", "федеральный архив", "ведомственный архив",
                          "ЗАГС", "ведомство") and not og.get("access_rights"):
        warnings.append(f"organization {oid_}: не записано, КТО ВПРАВЕ спрашивать "
                        "и надо ли подтверждать родство — а это решает, "
                        "имеет ли смысл письмо вообще")
    try:
...
# --- research_priority: приоритет 1 обязан иметь ход -------------------------
# 🔴 ПРОВЕРЯЕТСЯ ТОЛЬКО ОДИН КОНЕЦ ШКАЛЫ, И ЭТО НЕ ЛЕНЬ, А ИЗМЕРЕНИЕ.
# `research_priority: 1` означает «узел двигает дерево ПРЯМО СЕЙЧАС» — утверждение
# о настоящем времени, и если ни одна живая задача человека не называет, оно пустое.
# Прочие ступени (2…5) — «дальше по убыванию», то есть про когда-нибудь; человек
# с приоритетом 3 и без задачи ничего не обещает, и ошибкой это не является.
# Он виден в витрине долгом «Людей с желаниями, но без живой задачи».
#
# 🔴 А ОБРАТНАЯ СТОРОНА — «приоритет 0, а живая задача есть» — НАМЕРЕННО НЕ ПРОВЕРЯЕТСЯ,
# и вот почему (измерено 2026-08-14, 28 случаев разобрано). `target_people` говорит
# «задача КАСАЕТСЯ этого человека», а не «его исследуют ради него самого», и разница
# не декоративная:
#   · однофамилец-контроль, заведённый чтобы РАЗВЕСТИ двух тёзок, стоит в задаче
#     о нашем предке и обязан иметь приоритет 0;
#   · отсоединённое гнездо стоит в задаче, которая его отсоединение проверяет;
#   · задача-разбор («пройти по заготовкам и решить, нужен ли ход») называет всех
#     разом — и, будь такое правило, разом «повышала» бы им приоритет.
# Из 28 случаев одиннадцать оказались третьего рода, и породила их задача, заведённая
# за час до проверки. ⇒ пока у `target_people` нет РОЛИ, приоритет 0 с задачей —
# законное сочетание, а не расхождение.
_live_target = {pid for t in tasks
                if t.get("status") in ("pending", "in_progress", "blocked")
                for pid in (t.get("target_people") or [])}
for p in people:
    if p.get("research_priority") == 1 and not p.get("superseded_by") \
            and p["id"] not in _live_target:
        errors.append(
            f"person {p['id']}: research_priority=1 («двигает дерево прямо сейчас»), "
            f"а ни одна живая задача его не называет — приоритет объявляет работу, "
            f"которой нет. Либо назвать его в target_people хода, либо понизить")
...
# ⚠️ Дефляция приоритета 1 (мета-аудит 2026-08-18). Приоритет 1 значит
# «следующий заход начинается с этого», и таких не бывает три десятка:
# к моменту разбора p1 стояло у 34 задач из 119 pending — треть очереди,
# и «прямо сейчас» перестало отличаться от «важно вообще». Порог мягкий
# и общий на все каналы: превышение — предупреждение, а не ошибка,
# потому что верхняя планка — решение владельца, а не свойство данных.
_p1_all = [t["id"] for t in tasks
           if t.get("status") == "pending" and t.get("priority") == 1]
if len(_p1_all) > 10:
    warnings.append(
        f"задач приоритета 1 — {len(_p1_all)} (порог 10): «прямо сейчас» перестаёт "
        f"ранжировать. Пересортируйте: p1 — это «следующий заход начинается с этого»")
...
# 🔴 КУЗЕННЫЙ БРАК ЛОМАЕТ ОДНО ЧИСЛО НА ЧЕЛОВЕКА, И ЭТО НЕ ОШИБКА ДАННЫХ.
# `generation` меряет ГЛУБИНУ ОТ КОРНЯ, а не биологическое поколение. Пока корень
# приходит к предку одним путём, разницы нет. Как только предок оказывается общим
# для мужа и жены — а в деревне, где все дворы одной фамилии, это норма, — путей
# становится два, и они разной длины: муж отстоит от общего предка на четыре шага,
# жена на три. Тогда никакого числа, верного сразу для обеих ветвей, не существует,
# и требовать его — значит требовать, чтобы граф был деревом.
# ⇒ Такое ребро объявляется в meta.generation_skew с причиной. Оно остаётся полноценным
# ребром графа: связность, видимость, каскад допущений и счётчики считают его как все.
# Из расчёта глубины оно исключается — по нему счётчик не идёт и о нём не спорит.
# ⚠️ Объявлять надо РЕБРО, а не человека: расхождение живёт на пути, а не в узле,
# и молчаливое «поставим число покрупнее» увело бы за собой целую ветвь.
_skew = graph["meta"].get("generation_skew") or {}
...
_rel_ids = {r["id"] for r in rels}
for _e in _skew:
    if _e not in _rel_ids:
        errors.append(f"meta.generation_skew: {_e} — нет такой связи")
    elif not str(_skew[_e]).strip():
        errors.append(f"meta.generation_skew: {_e} объявлен без причины")
if _skew:
    print(f"⭐ рёбер с объявленным сдвигом поколений (кузенные браки): {len(_skew)}")
for p in people:
...
if _stale_prose:
    # ⚠️ До 2026-08-18 здесь печатались ВСЕ id разом — стена из ~90 имён в каждом
    # прогоне. По собственному правилу проекта неустранимое здесь-и-сейчас
    # предупреждение — фон, а фон прячет настоящие сигналы (ровно так в 2026-08
    # утонуло предупреждение о поколениях супругов). Список никуда не делся —
    # его печатает --stale-prose, а перечитка идёт при следующем касании карточки.
    if "--stale-prose" in sys.argv:
        warnings.append(f"биография написана по другим данным ({len(_stale_prose)}): "
                        f"{', '.join(_stale_prose)} — после сверки "
                        f"`validate.py --stamp-prose <id …>`")
    else:
        warnings.append(f"биографий, написанных по другим данным: {len(_stale_prose)} "
                        f"(первые: {', '.join(_stale_prose[:5])}…) — полный список "
                        f"`validate.py --stale-prose`, после сверки `--stamp-prose <id …>`")
...
def write_status():
    ...
    # --- Второй документальный контур (мета-аудит 2026-08-18) --------------
    # Почти все документальные основания графа — метрические книги одного
    # корпуса, прочитанные одной расшифровкой и одними глазами: ошибки таких
    # оснований КОРРЕЛИРОВАНЫ, а слепое пятно корпуса невидимо по устройству
    # (провалы фонда находились уже трижды). На день заведения метрики
    # держали 234 документальных основания из 235 — все восемь линий.
    _line_rows = []
    for fam, data in sorted(rmap["family_resources"].items()):
        _ppl = set(data.get("people") or [])
        _genres, _n_conf = set(), 0
        for r in rels:
            if r["confidence"] != "confirmed" or r["type"] != "parent_child":
                continue
            if r.get("parent") in _ppl and r.get("child") in _ppl:
                _n_conf += 1
                for e in (r.get("evidence") or []):
                    if isinstance(e, dict) and e.get("role") == "joint_mention":
                        _genres.add((src_by_id.get(e["src"]) or {}).get("type", "?"))
        _other = sorted(_genres - {"metric_book"})
        _line_rows.append((data.get("display_name", fam), _n_conf, _genres, _other))
    _mono_n = sum(1 for _nm, _c, _g, _o in _line_rows if not _o)
    add("### Второй документальный контур: на каких жанрах стоят линии")
    add("")
    add("Ошибки одного корпуса коррелированы: один фонд, одна расшифровка, одни глаза.")
    add("Линия получает второй контур, когда хотя бы одно её confirmed-ребро держит")
    add("документ НЕ-метрического жанра — ревизию, исповедную роспись, ЗАГС.")
    add("")
    for _nm, _cnt, _genres, _other in _line_rows:
        if _other:
            _mark = "✅ " + ", ".join(_other)
        elif "metric_book" in _genres:
            _mark = "🔴 только метрики"
        else:
            _mark = "⚠️ документального жанра нет вовсе — линия на памяти"
        add(f"- {_nm}: {_cnt} confirmed-рёбер — {_mark}")
    add("")
    add(f"**Линий без второго контура: {_mono_n} из {len(_line_rows)}.** Каждый заказ")
    add("ревизии или исповедной росписи из очереди двигает это число.")
    add("")
    ...
    # ⚠️ Доля мета-работы (мета-аудит 2026-08-18): к моменту разбора очередь
    # на ~50 % обслуживала аппарат, и дрейф был виден только аудиту со стороны.
    # Признак задачи «про аппарат»: ни одного человека и ни одной гипотезы.
    _meta_pending = [t["id"] for t in tasks
                     if t.get("status") == "pending"
                     and not (t.get("target_people") or [])
                     and not (t.get("resolves_hypotheses") or [])]
    _pend_n = sum(1 for t in tasks if t.get("status") == "pending")
    add(f"- Задач про аппарат (ни людей, ни гипотез): **{len(_meta_pending)}** "
        f"из {_pend_n} pending — растёт ⇒ очередь обслуживает сама себя")
    ...
    # 🔴 ДОЛГ РАЗДЕЛЁН НА ДВА 2026-08-15, И ЭТО НЕ КОСМЕТИКА. Одно число мерило два разных
    # положения дел: у ПРЕДКА корня желание без хода — настоящий долг, потому что предки
    # и есть предмет исследования; у боковой родни то же самое — список пожеланий, и он
    # может жить годами, ничего не значит и никого не торопит.
    # ⚠️ Пока числа были слиты, долг выглядел неподъёмным (84 человека) и потому не брался
    # ни разу. Разделив, увидели: предков среди них ДВЕНАДЦАТЬ, и все двенадцать закрылись
    # семью ходами за один заход. Остальные 71 — граница охвата (решение владельца
    # 2026-08-10: узлом заводятся предки, их дети и потомки корня).
    # ⇒ Первое число держим к нулю. Второе печатаем как справку и к нулю НЕ гоним.
    # Та же половинчатость, что у `visible`: обязательно только для предков.
    _wish_no_task = [p["id"] for p in people
                     if str(p.get("research_wishes") or "").strip() and p["id"] not in _targeted]
    _wish_anc = [i for i in _wish_no_task if i in _anc]
    _wish_rest = [i for i in _wish_no_task if i not in _anc]
    add(f"- 🔴 ПРЕДКОВ с желаниями, но без живой задачи: **{len(_wish_anc)}**"
        + (f" — {', '.join(sorted(_wish_anc)[:8])}"
           f"{'…' if len(_wish_anc) > 8 else ''}" if _wish_anc else " — чисто")
        + ". Предки и есть предмет исследования: желание без хода тут настоящий долг")
    add(f"- Прочих людей с желаниями, но без живой задачи: **{len(_wish_rest)}** — "
        f"боковая родня и кандидаты. Это список пожеланий, а не долг: граница охвата "
        f"проекта проходит по предкам, их детям и потомкам корня")
    # 🔴 Заготовка (`stub`) — это ОБЪЯВЛЕННЫЙ ДОЛГ: человек заведён, чтобы к нему можно
    # было привязать связь и источник, а исследования по нему не велось. Заготовка,
    # в которую не целит ни одна задача, ничем не отличается от забытой. До 2026-08-14
    # витрина печатала про заготовки одно число в скобке заголовка и молчала о том,
    # что с ними делать; заведено по вопросу владельца.
    _stub_all = [p for p in people if p.get("stub") and not p.get("superseded_by")]
    _stub_notask = sorted(p["id"] for p in _stub_all
                          if not any(p["id"] in (tk.get("target_people") or [])
                                     for tk in tasks))
    # ⚙️ Приоритет ≥2 без хода — не ошибка, а долг: «когда-нибудь» ничем не обеспечено.
    # Ошибкой является только приоритет 1 (см. проверку выше): он про настоящее время.
    _prio_notask = sorted(p["id"] for p in people
                          if (p.get("research_priority") or 0) >= 2
                          and not p.get("superseded_by")
                          and p["id"] not in _targeted)
    _prio_any = [p for p in people if (p.get("research_priority") or 0) >= 2
                 and not p.get("superseded_by")]
    # Делится по тому же основанию, что и долг по желаниям (см. ниже): у предка объявленный
    # приоритет без хода — расхождение, у боковой родни — просто отметка «когда-нибудь».
    _prio_anc = [i for i in _prio_notask if i in _anc]
    add(f"- 🔴 ПРЕДКОВ с объявленным приоритетом, но без хода: **{len(_prio_anc)}**"
        + (f" — {', '.join(sorted(_prio_anc)[:8])}" if _prio_anc else " — чисто"))
    add(f"- Прочих с приоритетом ≥2 без хода: **{len(_prio_notask) - len(_prio_anc)}**"
        f" из {len(_prio_any)} — не ошибка (2…5 это «когда-нибудь»), но и не работа")
    add(f"- Заготовок (`stub`) без единой задачи: **{len(_stub_notask)}** из {len(_stub_all)}"
        + (f" — {', '.join(_stub_notask[:8])}{'…' if len(_stub_notask) > 8 else ''}"
           if _stub_notask else " — чисто")
        + ". Заготовка — объявленный долг: узел заведён ради чужой связи, а сам не исследован")

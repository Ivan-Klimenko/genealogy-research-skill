...
if CAPT:
    _OURS = _re.compile(
        ...
        # ⚠️ 2026-08-12 ТРИ ФОРМУЛИРОВКИ СУЖЕНЫ. «стояло», «прежде», «числился»
        # сами по себе ловят речь о ЛЮДЯХ и СЕЛЕНИЯХ: «в починке стояло пять дворов»,
        # «он умер прежде неё», «деревня числилась за Тектубаевским районом». Три
        # неустранимых предупреждения — это фон, а фон прячет сигнал.
        r'|прежде (?:здесь|стояло|так|считал|числил)|прежн\w+ (?:верси|прочтени|выписк|подпис)'
        r'|(?:здесь|тут|раньше здесь) стояло|стояло «|держал\w+ отцом'
        r'|числил\w+ (?:в графе|мужем|женой|за нами)|считал\w+ (?:по выписке|прежде)'
    ...
    # ⭐⭐ 2026-08-12: ПРИЗНАК РАСШИРЕН НА ВСЕ ПЕЧАТАЕМЫЕ ПОЛЯ И ПОЛУЧИЛ АДРЕС.
    # 🔴 До этого дня он смотрел только подписи и печатал 52 из 244 в каждом прогоне —
    # три месяца подряд, при том что подписи несколько раз чистили руками. Причина
    # была не в дисциплине: у подписи не было непечатаемой пары, и удовлетворить
    # признак можно было только СТЕРЕВ знание. Неустранимое предупреждение — это фон,
    # а фон прячет сигнал не хуже молчания.
    # ⇒ Заведено поле `research` у листа (пара к `text`), и признак теперь говорит,
    # КУДА переносить, а не только что не так. Заодно он смотрит `role` и `biography`:
    # правило стояло у биографии, а на подписи не перенеслось само.
    def _flag(where, txt, home):
        hits = sorted({m.group(0).lower() for m in _OURS.finditer(txt or '')})
        if hits:
            findings['22. печатаемое поле рассказывает о нашей работе'].append(
                f"{where}: {', '.join(hits[:5])}  → перенести в `{home}`")

    for k, v in CAPT.items():
        _flag(f'folios/{k}.text', v.get('text'), 'research')
    for _p in G['people']:
        _flag(f"people/{_p['id']}.role", _p.get('role'), 'notes')
        _flag(f"people/{_p['id']}.biography", _p.get('biography'), 'notes')
...
# --- 24. номер записи в пересказе против реестра листов (err_002) ------------
# 🔴 Сшивка соседних записей разворота (класс err_002) до сих пор ловилась
# только косвенно — через имена и через отсылки «того жъ починка». А у неё
# есть прямой отпечаток: пересказ называет КООРДИНАТУ листа и рядом НОМЕР
# записи, и если этот номер не значится в `records` того же листа, то либо
# номер взят у соседа, либо реестр прочитан неверно. Оба исхода стоят чтения.
#
# ⚠️ Мерка «сколько записей на листе» этот класс НЕ ловит: найденный случай
# сидел на развороте, где запись ОДНА, — склеились графы внутри одной строки
# (рукописное «М.» перед словом «Декабрь» прочитано машиной как «21» и принято
# за день рождения, а год взят из пометы в последней графе той же строки).
#
# ⚠️ И два подвоха разбора, на которых первая версия шумела вдвое:
#   ① «лл. 189об.—190» и «ск.164» — не номера записей, их надо вырезать
#      ДО поиска, иначе всякий лист даёт ложное попадание;
#   ② окно после координаты цепляет номер, относящийся к СЛЕДУЮЩЕЙ названной
#      координате. Поэтому окно обрывается на ближайшей следующей координате,
#      а не на фиксированной длине.
if CAPT:
    try:
        sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent))
        import folios as _F
        _have = set(CAPT) | {p.stem for p in (D / 'scans').rglob('*')
                             if p.suffix.lower() in ('.jpg', '.jpeg', '.png')}
        _coord = _F.by_coord(_have)
        _STEM = _re.compile(r'^d(\d{1,4})_sk(\d{1,4})')
        _CO = _re.compile(r'д\.\s?(\d{1,4})[^\d]{0,12}ск(?:ан)?\.?\s?(\d{1,4})')
        _CUT = _re.compile(r'лл?\.\s?\d{1,4}\s?об\.?\s?[—-]\s?\d{1,4}'
                           r'|ск(?:ан)?\.?\s?\d{1,4}|д\.\s?\d{1,4}')
        _RECNO = _re.compile(r'(?:№|запись|записи|зап\.)\s*№?\s*(\d{1,3})', _re.I)
        # ⚠️ ПЕРВЫЙ ПРОГОН РАССКАЗАЛ О ПРИЗНАКЕ: оба попадания оказались местами,
        # где чужой номер процитирован НАРОЧНО — разбором уже найденной ошибки
        # («помета „умеръ 1866 г. Ав. 6. № 57" стоит на строке записи № 107,
        # а не при нашей»). Такой разбор и есть то, чего признак добивается.
        #
        # 🔴 И ВТОРОЙ ПРОГОН РАССКАЗАЛ О НЁМ ЕЩЁ БОЛЬШЕ. Первая попытка глушить
        # такие места по смыслу — «исправлено», «ошибка пересказа», «принадлежит
        # записи» — провалилась на контрольном примере: подложенный заведомо
        # ложный номер признак ПРОМОЛЧАЛ, потому что рядом стоял разбор той же
        # ошибки со словом «исправлены». Глушитель по смыслу выключает признак
        # ровно там, где номера и водятся гуще всего.
        # ⇒ Молчание покупается ЯВНОЙ пометой, а не подходящей формулировкой, —
        # тот же приём, что у err_003 («АНТЕЦЕДЕНТ ОТСЫЛКИ РАЗРЕШЁН»). Помета
        # гасит СВОЁ окно целиком: окно и так обрывается на следующей координате,
        # то есть говорит об одном листе, а помета — прямое заявление автора
        # «здесь чужой номер назван нарочно». Разрешение записывается в источник,
        # и признак сам показывает остаток.
        _EXPLAINED = _re.compile(r'ЧУЖОЙ НОМЕР НАЗВАН НАРОЧНО')
        for _s in S.values():
            _stems = [st for st in _F.source_folios(_s, _have, _coord, project=B,
                                                    folios=CAPT)
                      if isinstance(CAPT.get(st), dict) and CAPT[st].get('records')]
            if not _stems:
                continue
            _bc = {}
            for _st in _stems:
                _m = _STEM.match(_st)
                if _m:
                    _bc[(int(_m.group(1)), int(_m.group(2)))] = _st
            _txt = str(_s.get('data_extracted') or '')
            _marks = list(_CO.finditer(_txt))
            for _i, _m in enumerate(_marks):
                _st = _bc.get((int(_m.group(1)), int(_m.group(2))))
                if not _st:
                    continue
                _stop = _marks[_i + 1].start() if _i + 1 < len(_marks) else len(_txt)
                _win = _CUT.sub(' ', _txt[_m.end():min(_stop, _m.end() + 260)])
                if _EXPLAINED.search(_win):
                    continue
                _recs = set(CAPT[_st]['records'])
                _miss = sorted(n for n in {int(x) for x in _RECNO.findall(_win)}
                               if n not in _recs)
                if _miss:

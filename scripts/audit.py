...
# ── 18. год записи снят с шапки дела, где шапкам верить нельзя ───────────────
# 🔬 РАЗБОР 2026-08-07: погодная раскладка целой гипотезы стояла на машинных
# шапках д.320, где соседние развороты дают 1876, 1873, 1870, 1878. Признак
# ищет источники, называющие «книгу на такой-то год» в делах с шумными шапками.
RE_BOOKYEAR = re.compile(r'(?:кн\.?|книг\w*)\s*на\s*(1[6-9]\d\d)', re.I)
if CS:
    for s in S.values():
        txt = str(s.get('archive_ref') or '') + ' ' + str(s.get('data_extracted') or '')
        yrs = set(RE_BOOKYEAR.findall(txt))
        if not yrs:
            continue
        if re.search(r'титульн|с описи|по описи|глазами', txt, re.I):
            continue
        for n in sorted(case_numbers(str(s.get('archive_ref') or ''))):
            c = CS.get(str(n))
            if c and c.get('headers_noisy'):
                findings['18. год записи снят с шумной шапки дела'].append(
                    f"{s['id']} ← д.{n}: назван год {sorted(yrs)}, а шапки этого дела "
                    f"ненадёжны (опись даёт {c.get('years_opis')})")
                break

...
def attach(graph, sources_doc, folios, have, project=None):
    ...
    (поле `srcs` записи и подсказки в `caption_worklist.py`). На живом проекте
    из 426 привязок 411 берут `srcs` именно оттуда, а явное поле `sources`
    заполнено у одного листа из 189. ⇒ Удалить парсер значило бы снять с карточек
    ссылки на документы. Координата больше НЕ РЕШАЕТ, чей это лист, — она только
    подписывает, где о нём написано.
    """
